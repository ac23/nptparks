...
.factory('Directory', function($cordovaSQLite) {
  ...
  return {
    ...
    var query = "SELECT id, type, lat, lng, name, img, GROUP_CONCAT(use) as categories, comments FROM "+type+" WHERE id="+viewID+" GROUP BY name";
        ...
        $cordovaSQLite.execute(db, query, []).then(function(res) {
            if(res.rows.length > 0) {
				...
				var lng = document.getElementById('currentLng').innerHTML;
				for(var i = 0; i < res.rows.length; i++) {
					console.log('row: '+res.rows.item(i).name);
                	...
                	parks.push(row);
                	console.log('row2: '+res.rows.item(i).name);
                	return row;
                }
            } else {
                console.log("No results found");
